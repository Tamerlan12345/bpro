...
2.  Код для **ЕДИНОЙ** диаграммы flowchart TD на языке Mermaid.js, которая полностью описывает весь процесс.
...
### Завершающее событие и результаты:
...
- **ЕДИНСТВЕННАЯ СХЕМА:** Весь процесс должен быть представлен в виде ОДНОЙ диаграммы \`flowchart TD\`. Не создавай несколько диаграмм.
- **СИНТАКСИС И СТИЛЬ:**
...
- **ФИГУРЫ:**
    ...
    - ЦИЛИНДР \`id[("Текст")]\`: Для баз данных, CRM, хранилищ (включая "КИАС", "БД").
...
- **СТИЛИЗАЦИЯ:** Обязательно добавляй стили для ключевых фигур в конце кода. Пример:
  \`\`\`
  classDef decision fill:#f9f,stroke:#333,stroke-width:2px;
  classDef database fill:#ccf,stroke:#333,stroke-width:2px;
  class id1,id2 decision;
  class id3 database;
  \`\`\`
...
"mermaidCode": "..." // Здесь Mermaid-код, представляющий одну-единственную схему
}
...
Ответ должен содержать ТОЛЬКО ИСПРАВЛЕННЫЙ код Mermaid.js, без объяснений и markdown.`;
    ...
    async function generateDiagramFromText(processDescription) {
        const prompt = `Ты — элитный бизнес-аналитик. Твоя задача — взять описание процесса от пользователя и превратить его в код для **ЕДИНОЙ** диаграммы flowchart TD на языке Mermaid.js.

Ты должен СТРОГО следовать этим правилам:
- **ЕДИНСТВЕННАЯ СХЕМА:** Весь процесс должен быть представлен в виде ОДНОЙ диаграммы \`flowchart TD\`. Не создавай несколько диаграмм.
- **Направление:** Всегда используй 'flowchart TD'.
- **Линии:** Все связи должны быть ПРЯМЫМИ (\`-->\`).
- **Экранирование:** Специальные символы внутри текста экранируй HTML-кодами (например, #quot; для кавычек).
- **Фигуры:** Используй ПРЯМОУГОЛЬНИК \`id["Текст"]\` для действий, РОМБ \`id{"Текст"}\` для решений, ЦИЛИНДР \`id[("Текст")]\` для баз данных (включая "КИАС", "БД") и ДОКУМЕНТ \`id>Текст]\` для документов.
- **КЛЮЧЕВОЕ ПРАВИЛО:** Весь текст внутри фигур ДОЛЖЕН быть заключен в стандартные двойные кавычки ASCII (\`"\`).
- **Стилизация:** Обязательно добавляй стили для ключевых фигур в конце кода.
