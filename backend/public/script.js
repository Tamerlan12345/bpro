...
- ВАЖНО: СТРОГАЯ ПРИВЯЗКА ЛОГИЧЕСКИХ БЛОКОВ (БАЗА):
  ...
  5) База данных (БД, КИАС, хранилище, платформа АФК, база данных ИИ) = строго <bpmn2:dataStoreReference> (цилиндр). Размещай её РЯДОМ с соответствующей задачей (сбоку, смещение по X на 200px). Связывай с task через <bpmn2:dataOutputAssociation> (task -> БД) или <bpmn2:dataInputAssociation> (БД -> task) с явным BPMNEdge и di:waypoint координатами в DI-блоке. Это ОБЯЗАТЕЛЬНО — без BPMNEdge связь не будет видна. Обязательно генерируй BPMNShape (dc:Bounds) для каждой dataStoreReference.
...
ВАЖНО: СТРОГАЯ ПРИВЯЗКА ЛОГИЧЕСКИХ БЛОКОВ (БАЗА): Вход/выход = <bpmn2:startEvent>/<bpmn2:endEvent>, шаг = <bpmn2:task>, ссылка на процесс = строго <bpmn2:callActivity> (двойная рамка), логическое ИЛИ = <bpmn2:exclusiveGateway> (ветвь "Да" ВНИЗ, "Нет" ВПРАВО/ВЛЕВО), база данных (КИАС, платформа АФК, база ИИ) = <bpmn2:dataStoreReference> с BPMNShape И обязательным BPMNEdge (di:waypoint) для видимой связи с задачей, Документы (договоры, полисы) = разрешено и нужно <bpmn2:dataObjectReference>. Если есть роли, используй <bpmn2:laneSet>/<bpmn2:lane>.
...
ВАЖНО: СТРОГАЯ ПРИВЯЗКА ЛОГИЧЕСКИХ БЛОКОВ (БАЗА):
  ...
  5) База данных (БД, КИАС, хранилище, платформа АФК, база данных ИИ) = строго <bpmn2:dataStoreReference> (цилиндр). Размещай её РЯДОМ с соответствующей задачей (сбоку, смещение по X на 200px). Связывай с task через <bpmn2:dataOutputAssociation> (task -> БД) или <bpmn2:dataInputAssociation> (БД -> task) с явным BPMNEdge и di:waypoint в DI-блоке. Без BPMNEdge связь невидима. Обязательно генерируй BPMNShape (dc:Bounds) для каждой dataStoreReference.
