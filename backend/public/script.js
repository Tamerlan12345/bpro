...
- Для каждого элемента (task, startEvent, endEvent, gateway) и для каждой связи (sequenceFlow) ДОЛЖНЫ быть сгенерированы соответствующие элементы DI (BPMNShape с dc:Bounds и BPMNEdge с di:waypoint) с математически корректными координатами X и Y, чтобы схема выстраивалась слева-направо или сверху-вниз. 
- Без координат (DI) визуальный редактор не сможет отобразить схему! Прояви математическую точность. Отступ между блоками делай не менее 150px.
...
Обязательно включи блок <bpmndi:BPMNDiagram> и сгенерируй координаты (Bounds/waypoint) для всех элементов, чтобы диаграмма отображалась визуально. Отступ между элементами минимум 150px.
