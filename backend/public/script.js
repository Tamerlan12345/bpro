document.addEventListener('DOMContentLoaded', () => {
    ...
    async function generateProcessArtifacts(processDescription) {
        const prompt = `Ты — элитный бизнес-аналитик и, по совместительству, высокоточный генератор кода Mermaid.js. Твоя задача — взять описание процесса и превратить его в JSON-объект с двумя артефактами:
1.  \`standardDescription\`: Структурированное описание шагов процесса в формате Markdown.
2.  \`mermaidCode\`: Код для диаграммы 'flowchart TD' на языке Mermaid.js.

### ОБЩИЕ ПРАВИЛА:
- Ты должен СТРОГО следовать всем правилам. Любое отклонение — критическая ошибка.
- Твой ответ должен быть ИСКЛЮЧИТЕЛЬНО валидным JSON-объектом. Никаких лишних символов, текста или markdown-обёрток \`\`\`json\` до или после объекта.

### ПРАВИЛА ДЛЯ \`standardDescription\` (MARKDOWN):
1.  Используй следующий шаблон Markdown, додумывая детали на основе контекста:
    \`\`\`markdown
    ...
    * **Шаг [Номер]. [Название шага]**
    ...
    \`\`\`
2.  **ВАЖНО:** Если во входных данных есть секция \`### Предложения по улучшению:\`, ты должен использовать её пункты как руководство для улучшения и полного переписывания основного описания процесса. Интегрируй эти улучшения в итоговый \`standardDescription\`. Саму секцию \`### Предложения по улучшению:\` в финальный JSON включать НЕ НУЖНО.

### ПРАВИЛА ДЛЯ \`mermaidCode\` (ЖЕЛЕЗНЫЕ ПРАВИЛА):
1.  **НАПРАВЛЕНИЕ:** Диаграмма ВСЕГДА должна быть 'flowchart TD'.
2.  **ФИГУРЫ (СТРОГО ПО ЛЕГЕНДЕ):**
    * **ПРЯМОУГОЛЬНИК \`id["Текст"]\`**: ТОЛЬКО для стандартных шагов/действий.
    * **РОМБ \`id{"Текст"}\`**: ИСКЛЮЧИТЕЛЬНО для решений/условий/вопросов.
    * **ЦИЛИНДР \`id[("Текст")]\`**: ТОЛЬКО для баз данных/хранилищ/CRM.
    * **ДОКУМЕНТ \`id>Текст]\`**: ТОЛЬКО для документов/отчётов.
    * **ЗАПРЕЩЕНО:** Не используй другие типы фигур.
3.  **ТЕКСТ ВНУТРИ ФИГУР:**
    * **ГЛАВНОЕ ПРАВИЛО:** Абсолютно ВЕСЬ текст внутри ЛЮБОЙ фигуры ДОЛЖЕН быть заключен в двойные кавычки ASCII (\`"\`). ПРАВИЛЬНО: \`A["Шаг 1"]\`. НЕПРАВИЛЬНО: \`A[Шаг 1]\`.
    * **ЭКРАНИРОВАНИЕ:** Все спецсимволы в тексте, особенно кавычки, ДОЛЖНЫ быть экранированы как HTML-сущности (например, \`#quot;\`).
4.  **СВЯЗИ:**
    * **ТИП СТРЕЛКИ:** Используй ИСКЛЮЧИТЕЛЬНО прямую стрелку \`-->\`.
    * **ТЕКСТ НА СВЯЗЯХ:** \`A-- Да -->B\`.
5.  **СТИЛИЗАЦИЯ (ОБЯЗАТЕЛЬНЫЙ БЛОК):**
    * В конце кода ОБЯЗАТЕЛЬНО должен быть блок \`classDef\` для стилизации. Используй ЭТОТ шаблон без изменений:
        \`\`\`
        classDef decision fill:#E6E6FA,stroke:#333,stroke-width:2px;
        classDef database fill:#D3D3D3,stroke:#333,stroke-width:2px;
        classDef document fill:#FAFAC8,stroke:#333,stroke-width:2px;
        \`\`\`
    * Применяй классы к нужным фигурам: \`B{"Проверка?"}:::decision\`.
...
"${processDescription}"`;
    ...
    async function getFixedMermaidCode(brokenCode, errorMessage) {
        const prompt = `Ты — эксперт по Mermaid.js. Тебе дали код с ошибкой. Твоя задача — ИСПРАВИТЬ ЕГО, следуя строгим правилам.
...
Проанализируй ошибку и верни ИСПРАВЛЕННЫЙ код.

### ЖЕЛЕЗНЫЕ ПРАВИЛА ИСПРАВЛЕНИЯ:
1.  **НАПРАВЛЕНИЕ:** Диаграмма ОБЯЗАТЕЛЬНО должна быть 'flowchart TD'.
2.  **ФИГУРЫ:** Используй фигуры СТРОГО по легенде:
    * ПРЯМОУГОЛЬНИК \`id["Текст"]\` для действий.
    * РОМБ \`id{"Текст"}\` для решений.
    * ЦИЛИНДР \`id[("Текст")]\` для баз данных/систем.
    * ДОКУМЕНТ \`id>Текст]\` для документов.
3.  **ТЕКСТ ВНУТРИ ФИГУР:**
    * **ГЛАВНОЕ ПРАВИЛО:** Абсолютно ВЕСЬ текст внутри ЛЮБОЙ фигуры ДОЛЖЕН быть в двойных кавычках ASCII (\`"\`).
    * **ПРАВИЛЬНО:** \`A["Шаг 1"]\`. **НЕПРАВИЛЬНО:** \`A[Шаг 1]\`.
    * Все кавычки и спецсимволы в тексте ДОЛЖНЫ быть экранированы как HTML-сущности (например, \`#quot;\`).
4.  **СВЯЗИ:** Используй ИСКЛЮЧИТЕЛЬНО прямую стрелку \`-->\`. Текст на стрелках: \`A-- Да -->B\`.
5.  **СТИЛИ:** Убедись, что в конце есть ОБЯЗАТЕЛЬНЫЙ блок \`classDef\` и классы применены к нужным фигурам (\`id:::decision\`).
    \`\`\`
    classDef decision fill:#E6E6FA,stroke:#333,stroke-width:2px;
    classDef database fill:#D3D3D3,stroke:#333,stroke-width:2px;
    classDef document fill:#FAFAC8,stroke:#333,stroke-width:2px;
...
Ответ должен содержать ТОЛЬКО ИСПРАВЛЕННЫЙ и полностью валидный код Mermaid.js, без \`\`\`mermaid\` и других пояснений.`;
    ...
    async function generateDiagramFromText(processDescription) {
        const prompt = `Ты — высокоточный генератор кода для Mermaid.js. Твоя задача — преобразовать текстовое описание процесса в СТРОГО валидный код для диаграммы 'flowchart TD'.

### ЖЕЛЕЗНЫЕ ПРАВИЛА ГЕНЕРАЦИИ MERMAID:
Ты должен следовать этим правилам без исключений. Любое отклонение — это критическая ошибка.

1.  **НАПРАВЛЕНИЕ:** Диаграмма ВСЕГДА должна быть 'flowchart TD'.

2.  **ФИГУРЫ (СТРОГО ПО ЛЕГЕНДЕ):**
    * **ПРЯМОУГОЛЬНИК \`id["Текст"]\`**: ИСПОЛЬЗУЙ ТОЛЬКО для стандартных шагов и действий.
    * **РОМБ \`id{"Текст"}\`**: ИСПОЛЬЗУЙ ИСКЛЮЧИТЕЛЬНО для точек принятия решений, условий, вопросов (Да/Нет, Если, Проверить).
    * **ЦИЛИНДР \`id[("Текст")]\`**: ИСПОЛЬЗУЙ ТОЛЬКО для обозначения баз данных, хранилищ, CRM, систем учёта (например, "КИАС").
    * **ДОКУМЕНТ \`id>Текст]\`**: ИСПОЛЬЗУЙ ТОЛЬКО для обозначения документов, отчётов, счетов, заявок.
    * **ЗАПРЕЩЕНО:** Не используй другие типы фигур (круги, овалы и т.д.).

3.  **ТЕКСТ ВНУТРИ ФИГУР:**
    * **ГЛАВНОЕ ПРАВИЛО:** Абсолютно ВЕСЬ текст внутри ЛЮБОЙ фигуры (внутри \`""\`, \`{}\`, \`()\`, \`>\`) ДОЛЖЕН быть заключен в стандартные двойные кавычки ASCII (\`"\`).
    * **Пример ПРАВИЛЬНО:** \`A["Шаг 1"]\`, \`B{"Проверка?"}\`, \`C[("БД")]\`.
    * **Пример НЕПРАВИЛЬНО:** \`A[Шаг 1]\`.
    * **ЭКРАНИРОВАНИЕ:** Все специальные символы внутри текста, особенно кавычки, ДОЛЖНЫ быть экранированы как HTML-сущности. Например, \`id["Текст с #quot;кавычкой#quot;"]\`.

4.  **СВЯЗИ:**
    * **ТИП СТРЕЛКИ:** Используй ИСКЛЮЧИТЕЛЬНО прямую стрелку \`-->\`.
    * **ЗАПРЕЩЕНО:** Не используй пунктирные (\`-.->\`), толстые (\`==>\`) или любые другие типы стрелок.
    * **ТЕКСТ НА СВЯЗЯХ:** Текст на связях (например, "Да" или "Нет") пишется так: \`A-- Да -->B\`.

5.  **СТИЛИЗАЦИЯ (ОБЯЗАТЕЛЬНЫЙ БЛОК):**
    * В конце кода ОБЯЗАТЕЛЬНО должен быть блок \`classDef\` для стилизации фигур согласно легенде. Используй ЭТОТ шаблон без изменений:
        \`\`\`
        classDef decision fill:#E6E6FA,stroke:#333,stroke-width:2px;
        classDef database fill:#D3D3D3,stroke:#333,stroke-width:2px;
        classDef document fill:#FAFAC8,stroke:#333,stroke-width:2px;
        \`\`\`
    * Применяй эти классы к соответствующим фигурам. Пример: \`B{"Проверка?"}:::decision\`, \`C[("БД")]:::database\`.

6.  **ФОРМАТ ОТВЕТА:** Твой ответ должен быть ТОЛЬКО кодом Mermaid.js, без \`\`\`mermaid\` и других пояснений.
